...
func send(body string) {
	from := "kankanalasai@gmail.com"
	pass := "9849973284"
	to := "saitej1997@gmail.com"
...
func GetHardwareData() string {
	...
	html := "OS : " + runtimeOS
	html = html + "Total memory: " + strconv.FormatUint(vmStat.Total, 10) + " bytes " + "\n"
	html = html + "Free memory: " + strconv.FormatUint(vmStat.Free, 10) + " bytes" + "\n"
	...
	html = html + "Total disk space: " + strconv.FormatUint(diskStat.Total, 10) + " bytes " + "\n"
	html = html + "Used disk space: " + strconv.FormatUint(diskStat.Used, 10) + " bytes" + "\n"
	html = html + "Free disk space: " + strconv.FormatUint(diskStat.Free, 10) + " bytes" + "\n"
	...
	html = html + "CPU index number: " + strconv.FormatInt(int64(cpuStat[0].CPU), 10) + "\n"
	html = html + "VendorID: " + cpuStat[0].VendorID + "\n"
	html = html + "Family: " + cpuStat[0].Family + "\n"
	html = html + "Number of cores: " + strconv.FormatInt(int64(cpuStat[0].Cores), 10) + "\n"
	html = html + "Model Name: " + cpuStat[0].ModelName + "\n"
	html = html + "Speed: " + strconv.FormatFloat(cpuStat[0].Mhz, 'f', 2, 64) + " MHz " + "\n"

	for idx, cpupercent := range percentage {
		html = html + "Current CPU utilization: [" + strconv.Itoa(idx) + "] " + strconv.FormatFloat(cpupercent, 'f', 2, 64) + "\n"
	}

	html = html + "Hostname: " + hostStat.Hostname + "\n"
	html = html + "Uptime: " + strconv.FormatUint(hostStat.Uptime, 10) + "\n"
	...
	html = html + "OS: " + hostStat.OS + "\n"
	html = html + "Platform: " + hostStat.Platform + "\n"

	// the unique hardware id for this machine
	html = html + "Host ID(uuid): " + hostStat.HostID + "\n"

	for _, interf := range interfStat {
		html = html + "Interface Name: " + interf.Name + "\n"

		if interf.HardwareAddr != "" {
			html = html + "Hardware(MAC) Address: " + interf.HardwareAddr + "\n"
		}

		for _, flag := range interf.Flags {
			html = html + "Interface behavior or flags: " + flag + "\n"
		}

		for _, addr := range interf.Addrs {
			html = html + "IPv6 or IPv4 addresses: " + addr.String() + "\n"
